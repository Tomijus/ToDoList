...
namespace ToDoList.SQL
{
    class ToDoListMySql : SqlConnection, IRepository<ToDoItem>
    {
        ...
        {
            List<ToDoItem> retVal = new List<ToDoItem>();
            using (cmd = new MySqlCommand("SELECT * FROM ToDoList WHERE Task = @task", connection))
            {
                connection.Open();
                using (dr = cmd.ExecuteReader())
                {
        ...
        public List<ToDoItem> Sort()
        {
            ...
            using (cmd = new MySqlCommand("SELECT * FROM ToDoList ORDER BY @criteria ASC", connection))
            {
                connection.Open();
                using (dr = cmd.ExecuteReader())
                {
